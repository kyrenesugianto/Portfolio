...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        alpha = self.alpha
        next_state = self.computeValueFromQValues(nextState)
        "Q(s,a)←(1−α)⋅Q(s,a)+α⋅(r+γ⋅max(Q(s′,a))"
        updated_qVal = (1 - alpha) * qCurr + (alpha * (reward + (discount * next_state)))       
        self.values[(state, action)] = updated_qVal
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature in features.sortedKeys():
          weight_val = weight[feature]
          feature_val = features[feature]
          tot_qValue += weight_val * feature_val
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        diff = (reward + self.discount * nextQValue) - qValue
        alpha = self.alpha
        for feature in feat.sortedKeys():       
          "wi <- wi + α⋅difference⋅fi(s,a)"
          self.weights[feature] = self.weights[feature] + alpha * diff * feat[feature]
